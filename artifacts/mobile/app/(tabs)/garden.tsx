...
export default function GardenScreen() {
  ...
  const loadGarden = () => {
    ...
    fetch(`${BASE_URL}/api/plants/garden?userId=${user.userId}`)
      .then((r) => r.json())
      .then((data) => { if (Array.isArray(data)) setPlants(data.slice().reverse()); })
      .catch(() => {})
      .finally(() => setLoading(false));
  };

  useEffect(() => { loadGarden(); }, [user]);

  const handleDelete = (id: string) => {
    Alert.alert(t('removePlant'), t('removePlantDesc'), [
  ...
  return (
    <View style={[styles.container, { paddingTop: webTopPad }]}>
      <View style={[styles.header, { paddingTop: insets.top + 16 }]}>
        <Text style={[styles.title, isRTL && styles.textRTL]}>{t('myGarden')}</Text>
      ...
      {loading ? (
        ...
        <View style={styles.emptyContainer}>
          ...
          <Text style={[styles.emptyTitle, isRTL && styles.textRTL]}>{t('gardenEmpty')}</Text>
          <Text style={[styles.emptyText, isRTL && styles.textRTL]}>{t('gardenEmptyDesc')}</Text>
          <TouchableOpacity
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <PlantCard item={item} onDelete={() => handleDelete(item.id)} isRTL={isRTL} />
          )}
